...
class WebSocket{
      ...
      foreach($changed as $socket){
        if($socket==$this->master){
          ...
          if($bytes==0){ $this->disconnect($socket); }
				...
				$sent_by_user = $this->getuserbysocket($socket);
				foreach ($this->users as &$user) {
					
					if(!$user->handshake){ $this->dohandshake($user,$buffer); }
					else {
						//if($user->socket!=$sent_by_user->socket) {						
							$this->process($sent_by_user,$user,$this->unwrap($buffer));
						//}
					}
  ...
  function process($sent_by_user,$user,$msg){
    /* Extend and modify this method to suit your needs */
    /* Basic usage is to echo incoming messages back to client */
    $this->send($user->socket,$sent_by_user->socket."|".$msg);
		
  }

  function send($client,$msg){ 
    $this->say("> ".$msg." - ".count($this->users));
    $msg = $this->wrap($msg);
